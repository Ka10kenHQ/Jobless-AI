...
async def prepare_training_data_from_mongodb(languages, num_samples):
    ...
    if len(existing_examples) < num_samples:
        ...
        recent_jobs = await job_ops.get_recent_jobs(
            hours=24 * 365, limit=500
        )  # Last year
        ...
        for i, job in enumerate(recent_jobs):
            ...
            if job.language in languages:
                ...
                if any(
                    ...
                    for city in ["tbilisi", "batumi", "kutaisi", "rustavi", "gori"]
                ):
                    location_context = f"{job.location}, Georgia (country)"
                elif (
                    "georgia" in job.location.lower()
                    and "atlanta" not in job.location.lower()
                ):
        ...
        if new_examples:
            ...
            print(
                f"💾 Saved {saved_count} new training examples to MongoDB (including {len(georgia_examples)} Georgia-specific examples)"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train job search model with MongoDB data and automatic GPU optimization"
    )
    parser.add_argument(
        "--languages",
        nargs="+",
    ...
    parser.add_argument(
        "--show-gpu-info", action="store_true", help="Show detailed GPU information"
    )
    ...
    try:
        training_data = asyncio.run(
            prepare_training_data_from_mongodb(args.languages, num_samples)
        )
    ...
    for item in training_data:
        formatted_data.append({"input": item["input"], "output": item["output"]})
